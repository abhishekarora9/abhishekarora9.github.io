...
function App() {
  ...
  const handleLogin = async (ads_id, password) => {
    ...
    try {
      ...
      console.log("Login response headers:", Object.fromEntries(response.headers.entries())); // Debug log
  ...
  useEffect(() => {
    ...
    const fetchResultsStructure = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/results_structure`, {
          headers: getAuthHeaders()
        });
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    const fetchReprocessableFiles = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/reprocessable_files`, {
          headers: getAuthHeaders()
        });
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    const fetchOutputs = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/job_outputs/${jobId}`, {
          headers: getAuthHeaders()
        });
        const data = await res.json();
        ...
        for (const { key, label, type } of outputOrder) {
          if (data.outputs && data.outputs[key] && !shownOutputs[key]) {
            if (type === "text") {
              ...
              const textRes = await fetch(`${API_BASE_URL}/download/${jobId}/${key.replace("_path", "")}`);
              ...
              newHistory.push({
                ...
                content: (
                  <a
                    href={`${API_BASE_URL}/download/${jobId}/${key.replace("_path", "")}`}
                    target="_blank"
  ...
  const handleProcessExisting = async (s3Key) => {
    ...
    setShowChat(false);
    setStatus("processing");
    const res = await fetch(`${API_BASE_URL}/process_existing`, {
      method: "POST",
      headers: { 
        "Content-Type": "application/json",
        ...getAuthHeaders()
  ...
  const handleReprocess = async (s3Key) => {
    ...
    try {
          const res = await fetch(`${API_BASE_URL}/reprocess`, {
      method: "POST",
      headers: { 
        "Content-Type": "application/json",
        ...getAuthHeaders()
  ...
  const handleBatchReprocess = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/reprocess_batch`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          ...getAuthHeaders()
  ...
  const handlePreview = async (inputKey, output) => {
    ...
    try {
      const url = `${API_BASE_URL}/results/${inputKey}/${output}`;
  ...
  const handleDownloadPreview = (inputKey, filename) => {
    ...
    const url = `${API_BASE_URL}/results/${inputKey}/${filename}`;
                                  ...
                                  return (
                                    ...
                                    onClick={() => {
                                      if (isPreview) {
                                        ...
                                        window.open(`${API_BASE_URL}/results/${inputKey}/${output}`, '_blank');
                                      }
                                    }}
                                    >
                                      {/* Subtle accent line */}
